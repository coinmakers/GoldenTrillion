...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("GTRMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GoldenTrillionCashMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GoldenTrillionCashMiner started (POS=%s)\n", (fProofOfStake ? "true" : "false") );
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GoldenTrillionCash-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        //LogPrintf("Running GoldenTrillionCashMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
