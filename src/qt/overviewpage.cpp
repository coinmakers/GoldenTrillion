...
// Copyright (c) 2018-2020 The GTR developers
...
#define NEWS_URL "https://goldentrillion.com"
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::GTR)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount GTRAvailableBalance = balance - immatureBalance - nLockedBalance;

    // GTR Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // GTR labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, GTRAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // GTR Available
    bool showGTRAvailable = settingShowAllBalances || GTRAvailableBalance != nTotalBalance;
    bool showWatchOnlyGTRAvailable = showGTRAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showGTRAvailable || showWatchOnlyGTRAvailable);
    ui->labelBalance->setVisible(showGTRAvailable || showWatchOnlyGTRAvailable);
    ui->labelWatchAvailable->setVisible(showGTRAvailable && showWatchOnly);

    // GTR Pending
    bool showGTRPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyGTRPending = showGTRPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showGTRPending || showWatchOnlyGTRPending);
    ui->labelUnconfirmed->setVisible(showGTRPending || showWatchOnlyGTRPending);
    ui->labelWatchPending->setVisible(showGTRPending && showWatchOnly);
    ...
    // GTR Locked
    bool showGTRLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyGTRLocked = showGTRLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showGTRLocked || showWatchOnlyGTRLocked);
    ui->labelLockedBalance->setVisible(showGTRLocked || showWatchOnlyGTRLocked);
    ui->labelWatchLocked->setVisible(showGTRLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default (GTR)
    updateDisplayUnit();
}
...
{
    QString link="https://goldentrillion.com/";
